...
from apps.task.models.model_streamed import (
    Streamed,
    StreamedForStudent,
    StreamedForStudentSubmit,
)
...
class StreamedDao:
    ...
    def insert(
        ...
        try:
            cursor = conn.cursor()
            cursor.execute(
                sql, (streamed_name, streamed_text, streamed_limit, group_id)
            )
    ...
    def find_by_id(self, streamed_id: int) -> StreamedForStudentSubmit | None:
        ...
        try:
            ...
            # StreamedForStudentSubmitオブジェクトに変換
    ...
    def get_streamed_count(self, admin_id: str) -> int:
        ...
        group の作成者(課題の作成者)が一致するものだけを対象
        COUNT(*)で件数を数える
        """
        sql = """
